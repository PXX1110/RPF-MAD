...
_ATTACKED_CIFAR_10_DATASET_DIR = r'/home/group1/pxx/workplace/5Adversarial/MADS/MAD-C'
_ATTACKED_MNIST_DATASET_DIR = r'/home/group1/pxx/workplace/5Adversarial/MADS/MAD-M-R'
_ATTACKED_TinyImageNet_DATASET_DIR = r'/home/group1/pxx/workplace/5Adversarial/MADS/MAD-T'
...
class FewShotDataloader():
    def __init__(self,
                 ...
                 nTestNovel=1 * 6, # number of test examples for all the novel categories.
                 nTestBase=2 * 15, # number of test examples for all the base categories.
        ...
        max_possible_nKnovel = (self.dataset.attack_num_cats_base if self.phase=='train'
                                else self.dataset.attack_num_cats_novel)
        assert(nKnovel >= 0 and nKnovel < max_possible_nKnovel)
        self.nKnovel = nKnovel                                                           # 1
        self.mix = mix
        max_possible_nKbase = self.dataset.attack_num_cats_base
    ...
    def sampleImageIdsFrom(self, cat_id, sample_size=1):
        ...
        # without class
        # available_image_ids = self.dataset.attack_label2ind[cat_id]
        # assert(len(available_image_ids) >= sample_size)
        # img_id = np.array(random.sample(available_image_ids, sample_size))

        # with 10 class
        label2ind_keys = list(self.dataset.label2ind.keys())
        random.seed()  # This reseeds with the current system time or random source
        sample_keys = random.sample(label2ind_keys, 10)
        samples = {key: self.dataset.label2ind[key] for key in sample_keys}
        img_id = np.empty(shape=(0,sample_size))
        for i in range(len(samples)):
            label = [x for x in samples[sample_keys[i]] if x in self.dataset.attack_label2ind[cat_id]]
            img_id = np.vstack((img_id, np.array((random.sample(label, sample_size))))).astype(int)
        
        # with class
        # img_id = np.empty(shape=(0,sample_size))
        # for i in range(len(self.dataset.label2ind)):
        #     label = [x for x in self.dataset.label2ind[i] if x in self.dataset.attack_label2ind[cat_id]]
        #     img_id = np.vstack((img_id, np.array((random.sample(label, sample_size))))).astype(int)
        # Note: random.sample samples elements without replacement.
        return img_id

    def sample_Attack_Categories(self, cat_set, epoch, sample_size=1):
        """
        ...
        """
        if cat_set=='base':
            labelIds = self.dataset.attack_labelIds_base
        elif cat_set=='novel':
            labelIds = self.dataset.attack_labelIds_novel
        else:
    ...
    def sample_base_and_novel_categories(self, nKbase, nKnovel, epoch):
        """
        ...
        if self.is_eval_mode:
            if self.mix == True:
                assert(nKnovel <= self.dataset.attack_num_cats_novel)
                # sample from the set of base categories 'nKbase' number of base
                # categories.
                cats_ids = self.sample_Attack_Categories('base', epoch, nKbase) + self.sample_Attack_Categories('novel', epoch, nKnovel)
                ...
                # sample from the set of novel categories 'nKnovel' number of novel
                # categories.
                Knovel = sorted(self.sample_Attack_Categories('novel', epoch, nKnovel))
                Kbase = sorted(self.sample_Attack_Categories('base', epoch, nKbase))
        else:
            # sample from the set of base categories 'nKnovel' + 'nKbase' number
            # of categories.
            cats_ids = self.sample_Attack_Categories('base', epoch, nKnovel+nKbase)
    ...
    def sample_train_and_test_examples_for_attacks(
        ...
        nEvalExamplesPerClass = int(nTestNovel)  #   1
    ...
    def sample_episode(self, epoch):
        ...
        Kbase, Knovel = self.sample_base_and_novel_categories(nKbase, nKnovel, epoch)
    ...
    def createExamplesTensorData(self, examples):
        ...
        Returns:
            ...
            labels: a tensor of shape [nExamples] with the category label
                of each example.
        """
        data = [self.dataset[img_idx] for img_idx, _ in examples]
    ...
    def get_iterator(self, epoch=0):
        ...
        def load_function(iter_idx):
            Exemplars, Test, Kall, nKbase = self.sample_episode(epoch)
