...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu', default='6') # 0, 1, 2, 3, 4, 5, 6, 7 
    parser.add_argument('--network', type=str, default='Resnet18',
                            help='choose which embedding network to use. Resnet18, AlexNet, EfficientNet')
    parser.add_argument('--model', type=str, default=r'/home/users/pxx/workplace/5Adversarial/4Meta_AT_ResNet/results_ssl/01-27_20-59/best_checkpint.pkl')
    ...
    parser.add_argument('--victim', default='Meta_A-InfoNCE', choices=['simclr', 'byol', 'dino', 'mocov3', 'mocov2plus',
    ...
    log_dir = os.path.join(BASE_DIR, "..", "results_ssl", "Strong_MCL_robust_best_new_{}".format(opt.load), "{}_{}".format(opt.victim, opt.phase))
    # create save dir
    os.makedirs(log_dir, exist_ok=True) 
    log_file_path = os.path.join(log_dir, "MCL_{}_{}_attack_test_log.txt".format(opt.dataset, opt.phase))
    ...
    for i in range(len(ATK(opt.attack, opt.dataset))):
        ...
        if opt.load == 'SSL':
            ...
            if opt.victim in ['AdvCL', 'A-InfoNCE','Meta_AdvCL', 'Meta_A-InfoNCE']:
                ...
                do_normalize = 1
            model = resnet18(num_classes=10, do_normalize=do_normalize)
