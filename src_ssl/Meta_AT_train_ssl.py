...
if __name__ == "__main__":
    ...
    parser.add_argument('--labelsmoothvalue', default=0.1, type=float)   # 0.2
    parser.add_argument('--mixup', action='store_true')# whether use mixup , action='store_true'
    parser.add_argument('--mixup-alpha', default=1.4, type=float)        # 1.4
    ...
    log_file_path = os.path.join(log_dir, "{}_Meta_AT_train_lr_max_0.01_ls_0.1.txt".format(opt.dataset))
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        train_loss_avg, train_acc_avg, lr_s, optimizer, network = ModelTrainer.train_ssl(epoch ,dloader_train, network, criterion, opt_s, lr_schedule_s, opt.num_epoch, log_file_path, log_dir, opt)
        ...
        if (epoch) % opt.save_epoch == 0 or epoch >= 5:
            print("Waiting Test!")
            val_loss_avg, val_acc_avg = ModelTrainer.valid_ssl(epoch, dloader_val, network, criterion, optimizer, opt.num_epoch, log_file_path, log_dir, opt)
