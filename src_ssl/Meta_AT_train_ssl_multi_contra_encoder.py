...
from configs.experiment_config import CRITERTION, get_model, get_dataset_train, get_dataset_train_without_class, get_model_ACL
...
if __name__ == "__main__":
    ...
    # Group 1
    parser.add_argument('--num-epoch', type=int, default=15,  help='number of training epochs')
    ...
    parser.add_argument('--contrastive', action='store_true') # whether use label smoothing , default=True , action='store_true'
    ...
    parser.add_argument('--lr_maxe', default=0.005, type=float)
    parser.add_argument('--lr_maxf', default=0.005, type=float)
    ...
    # Group 3
    parser.add_argument('--dualBN', type=int, default=0, help='Whether to use dual BN module during fine-tuning')
    ...
    log_file_path = os.path.join(log_dir, "{}_Meta_Multi_AT_train_0.005_0.005_SGD_encoder_ce_15_wr.txt".format(opt.dataset))
    log(log_file_path, str(vars(opt)))
    network = get_model_ACL(opt, device)
    network.cuda()
    ...
    criterion_C = CRITERTION
    criterion_N = nt_xent
    if opt.labelsmooth:
        criterion_C = LabelSmoothingLoss(smoothing=opt.labelsmoothvalue)
    elif opt.contrastive:
        criterion_N = nt_xent
    # else:
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        train_loss_avg, train_acc_avg, opt_e, opt_f, network, encoder = ModelTrainer.train_ssl_encoder(epoch, dloader_train, network, encoder,
                                    criterion_C, criterion_N, opt_e, opt_f, lr_schedule_e, lr_schedule_f, opt.num_epoch, log_file_path, log_dir, opt)
        ...
        if (epoch) % opt.save_epoch == 0 or epoch >= 5:
            print("Waiting Test!")
            val_loss_avg, val_acc_avg = ModelTrainer.valid_ssl_multi(epoch, dloader_val, network, criterion_C, opt_e, opt_f,
