...
from configs.experiment_config import CRITERTION, get_model, get_dataset_train, get_dataset_train_without_class, load_SSL_model, load_victim
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu', default='6') #, 1, 2, 3'
    ...
    parser.add_argument('--lr_maxe', default=0.01, type=float)
    parser.add_argument('--lr_maxf', default=0.001, type=float)
    parser.add_argument('--lr-one-drop', default=0.01, type=float)
    parser.add_argument('--lr-drop-epoch', default=100, type=int)
    parser.add_argument('--lrdecay', default='base', type=str, choices=['intenselr', 'base', 'looselr', 'lineardecay'])
    parser.add_argument('--optimizer', default='Nesterov')    #, choices=['momentum', 'Nesterov', 'SGD_GC', 'SGD_GCC', 'Adam', 'AdamW']
    ...
    parser.add_argument('--victim', default='ACL', choices=['simclr', 'byol', 'dino', 'mocov3', 'mocov2plus',
                                 'nnclr', 'ressl', 'swav', 'vibcreg', 'wmse', 'deepclusterv2', 'robust_ACL',  'ACL', 'AdvCL', 'A-InfoNCE', 'DeACL',
                            'DynACL', 'DynACL++', 'DynACL-AIR', 'DynACL-AIR++', 'DynACL-RCS',])
    parser.add_argument('--init', type=str, default='None', choices=['None', 'Roli', 'Meta'])
    parser.add_argument('--mode', type=str, default='SLF', choices=['SLF', 'ALF', 'AFF', 'ACL','AInfoNCE','AInfoNCE_DRC'])
    ...
    log_file_path = os.path.join(log_dir, "{}_Meta_Multi_AT_train_new_opt.txt".format(opt.dataset))
    log(log_file_path, str(vars(opt)))
    network, _, _ = load_SSL_model(opt, device)
    network.cuda()
    encoder = nn.Sequential(*list(network.children())[:-1], nn.Flatten(1)).cuda()
    encoder.cuda()
    F = network.fc
    F.cuda()
    ...
    param_encoder = encoder.parameters()
    param_F = F.parameters()

    opt_e = torch.optim.Adam(encoder.parameters(), lr=0.0001, weight_decay=0.0008)
    opt_f = torch.optim.Adam(F.parameters(), lr=0.005, weight_decay=0.0008)
    lr_schedule_e = torch.optim.lr_scheduler.ExponentialLR(optimizer=opt_e, gamma=0.96)
    lr_schedule_f = torch.optim.lr_scheduler.ExponentialLR(optimizer=opt_f, gamma=0.96)

    # opt_e = set_optimizer(opt, opt.lr_maxe, param_encoder, optimizer=opt.optimizer, lr_schedule=opt.lr_schedule)
    # opt_f = set_optimizer(opt, opt.lr_maxf, param_F, optimizer=opt.optimizer, lr_schedule=opt.lr_schedule)
    # lr_schedule_e = set_lr_schedule(opt, opt.lr_maxe)
    # lr_schedule_f = set_lr_schedule(opt, opt.lr_maxf)
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        train_loss_avg, train_acc_avg, opt_e, opt_f, network = ModelTrainer.train_ssl_multi_opt_new(epoch ,dloader_train, network,
                                    criterion, opt_e, opt_f, lr_schedule_e, lr_schedule_f, opt.num_epoch, log_file_path, log_dir, opt)

        # train_loss_avg, train_acc_avg, e_optimizer, f_optimizer, network = ModelTrainer.train_ssl_multi(epoch, dloader_train, network, 
        #                                     criterion, e_optimizer, f_optimizer, opt.num_epoch, log_file_path, log_dir, opt)
        ...
        if (epoch) % opt.save_epoch == 0 or epoch >= 5:
            print("Waiting Test!")
            val_loss_avg, val_acc_avg = ModelTrainer.valid_ssl_multi(epoch, dloader_val, network, criterion, opt_e, opt_f,
                                                                      opt.num_epoch, log_file_path, log_dir, opt)
            # val_loss_avg, val_acc_avg = ModelTrainer.valid_ssl_multi(epoch, dloader_val, network, criterion, e_optimizer, f_optimizer,
            #                                                           opt.num_epoch, log_file_path, log_dir, opt)
