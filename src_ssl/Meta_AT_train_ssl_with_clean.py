...
if __name__ == "__main__":
    ...
    log_file_path = os.path.join(log_dir, "{}_Meta_Meta_AT_train_with_clean.txt".format(opt.dataset))
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        train_loss_avg, train_acc_avg, lr_s, optimizer, network = ModelTrainer.train_ssl_clean(epoch ,dloader_train, network, criterion, opt_s, lr_schedule_s, opt.num_epoch, log_file_path, log_dir, opt)
        ...
        if (epoch) % opt.save_epoch == 0 or epoch >= 5:
            print("Waiting Test!")
            val_loss_avg, val_acc_avg = ModelTrainer.valid_ssl_clean(epoch, dloader_val, network, criterion, optimizer, opt.num_epoch, log_file_path, log_dir, opt)
