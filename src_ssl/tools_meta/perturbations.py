...
def gaussian_noise(x, severity_params, image_size):
    x = np.array(x) / 255.0
    return (
        np.clip(x + np.random.normal(size=x.shape, scale=severity_params), 0, 1) 
    )


def shot_noise(x, severity_params, image_size):
    x = np.array(x) / 255.0
    return np.clip(np.random.poisson(x * severity_params) / severity_params, 0, 1) 
...
def gaussian_blur(x, severity_params, image_size):
    x = gaussian(np.array(x) / 255.0, sigma=severity_params, channel_axis=-1)
    return np.clip(x, 0, 1) 


def glass_blur(x, severity_params, image_size):
    x = np.uint8(gaussian(np.array(x) / 255.0, sigma=severity_params[0], channel_axis=-1)*255.0) 
    ...
    return (
        np.clip(gaussian(x  / 255.0, sigma=severity_params[0], channel_axis=-1), 0, 1)
    )
...
def defocus_blur(x, severity_params, image_size):
    ...
    channels = np.array(channels).transpose((1, 2, 0))  # 3ximage_sizeximage_size -> image_sizeximage_sizex3
...
#         return np.clip((x[..., [2, 1, 0]])/255, 0, 1)  # BGR to RGB
#     else:  # greyscale to RGB
#         return np.clip((np.array([x, x, x]).transpose((1, 2, 0)))/255, 0, 1)
...
def fog(x, severity_params, image_size):
    ...
    return np.clip(x * max_val / (max_val + severity_params[0]), 0, 1) 


def frost(x, severity_params, image_size):
    idx = np.random.randint(5)
    filename = [
        r"/home/users/pxx/workplace/5Adversarial/4Meta_AT_ResNet/data/filters/frost2.png",
        r"/home/users/pxx/workplace/5Adversarial/4Meta_AT_ResNet/data/filters/frost3.png",
        r"/home/users/pxx/workplace/5Adversarial/4Meta_AT_ResNet/data/filters/frost1.png",
        r"/home/users/pxx/workplace/5Adversarial/4Meta_AT_ResNet/data/filters/frost4.jpg",
        r"/home/users/pxx/workplace/5Adversarial/4Meta_AT_ResNet/data/filters/frost5.jpg",
        r"/home/users/pxx/workplace/5Adversarial/4Meta_AT_ResNet/data/filters/frost6.jpg",
    ][idx]
...
def contrast(x, severity_params, image_size):
    ...
    return np.clip((x - means) * severity_params + means, 0, 1) 
